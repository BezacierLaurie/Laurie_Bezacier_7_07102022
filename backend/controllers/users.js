...
exports.signup = (req, res, next) => { // 'signup' : fonction qui permet de CRYPTER le password
    // Pour 'HACHER' / CRYPTER le password (fonction asynchrone)
    bcrypt.hash(req.body.password, 10) // 'req.body.password' ('data'): password présent dans le corps de la requête - '10' ('salt'): nb de fois où l'algorythme de 'hachage' sera exécuté
...
exports.login = (req, res, next) => { // 'login' : fonction qui permet de VERIFIER si un 'user' existe dans la BdD ('MySQL') et si le MdP entré par le 'user' correspont à ce 'user'
    db.users.findOne({ where: { email: req.body.email } }) // '{}' = objet (sélecteur qui va servir de filtre) - 'req.body.email': valeur transmise par le 'user'
